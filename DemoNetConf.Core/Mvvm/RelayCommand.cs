...
namespace DemoNetConf.Core.Mvvm
{
    ...
    public class RelayCommand<T> : ObservableObject, ICommand
    {
        ...
        public virtual async void Execute(object parameter)
        {
            ...
            {
                if (parameter != null) ActionGeneric.Invoke((T)parameter);
                else ActionGeneric.Invoke(default);
            }
            if (ActionAsync != null) await ActionAsync.Invoke();
            if (ActionGenericAsync != null)
            {
                if (parameter != null) await ActionGenericAsync.Invoke((T)parameter);
                else await ActionGenericAsync.Invoke(default);
            }
            Processing = false;
        }
        #endregion
